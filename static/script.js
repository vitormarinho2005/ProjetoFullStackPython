document.addEventListener("DOMContentLoaded", () => {
    ...
    const mostrarToast = (msg, tipo="info", duracao=4000) => {
        const cores = { success:"bg-success text-white", info:"bg-info text-white", danger:"bg-danger text-white", secondary:"bg-secondary text-white" };
        ...
        bsToast.show(); toast.addEventListener("hidden.bs.toast",()=>toast.remove());
    };

    // --- Histórico ---
    const atualizarHistorico = () => {
        historico.innerHTML = "";
        if(!relatorios.length) { const vazio = document.createElement("li"); vazio.className="list-group-item text-muted text-center"; vazio.textContent="Nenhum relatório gerado ainda."; historico.appendChild(vazio); return; }
        relatorios.forEach((r,i)=>{
            const li=document.createElement("li");
            li.className="list-group-item d-flex justify-content-between align-items-center";
            li.innerHTML=`<span>${r.nome} - ${r.papel}</span>
                          <div>
    ...
    form.addEventListener("submit", async e=>{
        ...
        try {
            ...
            if(data.pdf_name){
                ...
                relatorios.push({nome,papel,url,pdf_name:data.pdf_name});
            ...
            } else { mostrarToast("❌ Erro ao gerar relatório","danger"); statusMsg.innerHTML="❌ Erro ao gerar relatório"; statusMsg.className="alert alert-danger mt-3"; }
        } catch(err){ mostrarToast("🚫 Falha na comunicação com o servidor","danger"); statusMsg.innerHTML="❌ Falha na comunicação com o servidor"; statusMsg.className="alert alert-danger mt-3"; }
    });
    ...
    historico.addEventListener("click", async e=>{
        ...
        if(acao==="baixar"){
            ...
            setTimeout(async ()=>{ await fetch(`/remover_pdf/${r.pdf_name}`,{method:"DELETE"}); mostrarToast("🗑️ Relatório removido automaticamente!","secondary"); },3000);
        }
        if(acao==="remover"){
            if(!confirm("Deseja realmente remover este relatório?")) return;
            relatorios.splice(i,1); localStorage.setItem("relatorios",JSON.stringify(relatorios)); atualizarHistorico();
